...
console.log('\x1b[33m[ WARN ]\x1b[37m » Change credited Fuck You By MR CHAND :p  :) '+ global.config.BOTNAME + ' đổi credits modules "' + this.config.name + '"');
return api.sendMessage('[ WARN ] Detect bot operator ' , event.threadID, event.messageID);
          }
                if (ctx.measureText(name).width < maxWidth) return resolve([name]);
                if (ctx.measureText('MADE BY MR CHAND').width > maxWidth) return resolve(null);
